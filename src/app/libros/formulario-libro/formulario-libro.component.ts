...
export class FormularioLibroComponent implements OnInit {
  ...
  constructor(
    public servicioAutores: AutoresService,
    public servicioToast: ToastController,
...
private registrar(){
  ...
  this.servicioLibros.post(libro).subscribe({
    next: () => {
      ...
      this.servicioToast.create({
        header: 'Se registró correctamente el libro',
        duration: 2000,
        color: 'success'
      }).then(t => t.present());
    },
    error: (e) => {
      console.error('Error al registrar Libros', e);
      this.servicioToast.create({
